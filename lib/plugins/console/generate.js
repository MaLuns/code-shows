...
class Generater {
    ...
    writeFile (path) {
        ...
        return finishedPromise.then(() => {
            ...
            return writeFile(dest, Buffer.concat(buffers)).then(() => {
                log.info('File Created: %s', cacheId);
                return true;
            });
        });
    }
    ...
    firstGenerate () {
        ...
        log.info('Files loaded in %s', interval);
        ...
        return stat(dist_dir).then(stats => {
            ...
            log.info('%d files generated in %s', result.length, interval);
        });
    }
}
...
module.exports = function () {
    ...
    return this.build().then(() => generater.firstGenerate()).then(() => {
        this.db.cache();
    });
};