...
const GIT_REPO_URL = 'https://github.com/MaLuns/code-shows-template.git';

async function init (name, path) {
    const target = resolve(path, name);
    if (existsSync(target) && readdirSync(target).length !== 0) {
        warn((`${target} not empty, please run \`code-shows init\` on an empty folder and then copy your files into it`));
