...
class BaseCore extends EventEmitter {
    ...
    fileExtension () {
        ...
        class _File extends File {
            render (options) {
                return ctx.render.run({ path: this.source }, options);
            }

            renderSync (options) {
                return ctx.render.runSync({ path: this.source }, options);
            }
        }

        _File.prototype.core = this;
        return _File;
    }
    ...
    _buildFile (type, path) {
        if (this._loadingFiles[path]) {
            return Promise.resolve();
        }

        this.emit('loadBefore', { type, path });

        const { base, File, context: ctx } = this;
        const { log } = ctx;
        this._loadingFiles[path] = true; // 标记文件处理中
        const source = join(base, path); // 文件绝对路径地址

        return Promise.reduce(this.loaders, (count, loader) => {
            const params = loader.pattern.match(path);  // 判断当前 loader 能否处理 当前文件
            if (!params) return count;

            const file = new File({ source, path, params, type });  // 处理文件
            return Reflect.apply(Promise.method(loader.load), ctx, [file]).thenReturn(count + 1); // 调用 build
        }, 0).then(count => {
            if (count) {
                log.info(`Load: ${path}`);
            }

            this.emit('loadAfter', { type, path });
        }).catch((err) => {
            log.error('Load failed: %s', source, { err });
        }).finally(() => {
            this._loadingFiles[path] = false;
        }).thenReturn(path);
    }
