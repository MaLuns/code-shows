...
function createThemeRoute (r, ctx) {
...
class CodeShows extends EventEmitter {
    ...
    constructor(base = process.cwd(), args = {}) {
        ...
        this.public_dir = join(base, 'publis') + sep; // 生成静态文件目录
    ...
    _generate (options = {}) {
        ...
        const data = this._runGenerators();

        this._routerReflesh(data);
    }
    ...
    _routerReflesh (runningGenerators, useCache) {
        ...
        return runningGenerators.map(result => {
            ...
            if (layout) {
                route.set(path, createThemeRoute(result, this));
            } else {
                route.set(path, data);
            }
