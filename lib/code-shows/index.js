...
const { Console, Loader, Deployer, Generator, Helper, Renderer, Middlewarer, Converter } = require('../extend');
const { version } = require('../../package.json');
const codeConfig = require('./code.config');
...
class CodeShows extends EventEmitter {
    ...
    async watch (cache = false) {
        this._watchCore = _.debounce(() => this._generate({ cache }), 100);

        await Promise.all([
            this.source.watch(),
            this.theme.watch()
        ]);
        this.source.on('loadAfter', this._watchCore);
        this.theme.on('loadAfter', this._watchCore);
    ...
    unwatch () {
        if (this._watchCore) {
            this.source.removeListener('loadAfter', this._watchCore);
            this.theme.removeListener('loadAfter', this._watchCore);
            this._watchCore = null;
        }
    ...
    _routerReflesh (runningGenerators, useCache) {
        ...
        return runningGenerators.map(result => {
            if (typeof result !== 'object' || result.route_url === null) {
                return undefined;
            }

            const path = route.format(result.route_url);
