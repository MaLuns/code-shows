...
const { Console, Builder, Deployer, Generator, Helper, Renderer } = require('../extend');
...
class CodeShows extends EventEmitter {
    ...
    // 
    load () {
        this.log.info('Loading source files...');
        return Promise.all([
            this.source.build(),
            this.theme.build()
        ]).then(() => {
    ...
    watch () {
        this._watchCompiler = _.debounce(() => this._generate({ cache: false }), 100);

        return Promise.all([
            this.source.watch(),
            this.theme.watch()
        ]).then(() => {
            this.source.on('buildAfter', this._watchCompiler);
            this.theme.on('buildAfter', this._watchCompiler);
            return this._generate({ cache: false });
        });
    }
